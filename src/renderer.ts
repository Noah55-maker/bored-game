...
let matrixInstancedLoc: number;
let brightnessAttribLoc: number;
let diffuseUniformInstanced: WebGLUniformLocation;
let lightDirectionUniformInstanced: WebGLUniformLocation;

let baseMatrix: number[];

let matrixPickingAttribLoc: number;
let idAttribLoc: number;
...
export class GamePiece {
    ...
    drawMultiple(
        ...
        for (let i = 0; i < numInstances; i++) {
            let specificMatrix = m4.translate(positionMatrix,
                MM_TO_IN * (xPositions[i] - (MAP_LENGTH - 1) / 2),
                0,
                MM_TO_IN * (yPositions[i] - (MAP_LENGTH - 1) / 2)
            );
    ...
    drawScrolling(
        ...
        for (let i = 0; i < numInstances; i++) {
            let specificMatrix = m4.translate(positionMatrix,
                MM_TO_IN * (xPositions[i] - (LAUNCH_SCREEN_MAP_LENGTH - 1) / 2),
                0,
                MM_TO_IN * (yPositions[i] - (LAUNCH_SCREEN_MAP_LENGTH - 1) / 2)
            );
...
function createInterleavedBufferVao(
    ...
    gl.vertexAttribPointer(positionAttribLocation, 3, gl.FLOAT, false,
        6 * Float32Array.BYTES_PER_ELEMENT, 0);
    gl.vertexAttribPointer(normalAttribLocation, 3, gl.FLOAT, true,
        6 * Float32Array.BYTES_PER_ELEMENT, // step
...
function createBufferVao(gl: WebGL2RenderingContext, buffer: WebGLBuffer, positionAttribLocation: number) {
    const vao = gl.createVertexArray();
    if (!vao) {
        throw new Error("Failed to allocate VAO for two buffers");
    }
...
function normalize(arr: number[]) {
    let squareSum = 0;
    for (let i = 0; i < arr.length; i++) squareSum += arr[i] * arr[i];

    const magnitude = Math.sqrt(squareSum);
    const normalized: number[] = [];

    for (let i = 0; i < arr.length; i++) normalized.push(arr[i] / magnitude);
...
async function importModel(assetName: string, vertexPosAttrib: number, vertexNormAttrib: number, vertPosPickingAttrib: number): Promise<GamePiece> {
    try {
...
export async function init(drawBoardScrolling: Function, drawBoardInstanced: Function) {
    ...
    const gamePieces = await Promise.all(ASSET_NAMES.map((assetName) =>
        importModel(assetName, vertexPositionAttributeLocation, vertexNormalAttributeLocation, vertexPositionPickingAttribLoc)
    ));

    gl.enable(gl.DEPTH_TEST);
    let lastTime = Date.now();
    let numFrames = 0;
    async function renderGameBoard(time: number) {
    ...
    async function renderLaunchScreen(time: number) {
        time *= 0.001; // convert to seconds
        const startTime = Date.now();
