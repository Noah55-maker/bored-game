...
class GamePiece {
    ...
    draw(gl: WebGL2RenderingContext, xPosition: number, yPosition: number,
        matrixUniform: WebGLUniformLocation, diffuseUniform: WebGLUniformLocation, time: number
    ) {
...
class Player {
    constructor(public troops: [Troop]) { }
 ...
 * @param gl 
 */
function drawBoard(gl: WebGL2RenderingContext, gamePieces: GamePiece[],
    matrixUniform: WebGLUniformLocation, diffuseUniform: WebGLUniformLocation, time: number) {
...
async function boredgame() {
    ...
    async function render(time: number) {
        ...
        if (matrixUniform === null || diffuseUniform === null) return;
